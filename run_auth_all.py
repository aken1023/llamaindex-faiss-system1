#!/usr/bin/env python
"""
LlamaIndex-FAISS 知識庫系統啟動腳本 (支持用戶認證)
此腳本將同時啟動前端和後端服務
"""
...
║                                                          ║
║  {Colors.YELLOW}支持用戶認證的個人知識庫系統{Colors.BLUE}                            ║
║                                                          ║
╚══════════════════════════════════════════════════════════╝{Colors.ENDC}
"""
    print(banner)
...
def check_requirements():
    ...
    # 檢查必要目錄
    if not Path("user_documents").exists():
        print(f"{Colors.YELLOW}[警告] 未找到 user_documents 目錄，正在創建...{Colors.ENDC}")
        Path("user_documents").mkdir(exist_ok=True)
    
    if not Path("user_indexes").exists():
        print(f"{Colors.YELLOW}[警告] 未找到 user_indexes 目錄，正在創建...{Colors.ENDC}")
        Path("user_indexes").mkdir(exist_ok=True)
...
def run_backend(python_path):
    """運行後端 API 服務器"""
    print(f"{Colors.BLUE}[後端] 啟動支持認證的 API 服務器...{Colors.ENDC}")
...
def cleanup(signum, frame):
    ...
    for process in processes:
        try:
            ...
            print(f"{Colors.RED}[錯誤] 關閉進程失敗: {e}{Colors.ENDC}")
    
    print(f"{Colors.GREEN}[系統] 服務已關閉{Colors.ENDC}")
    sys.exit(0)
...
def main():
    ...
    try:
        ...
        # 啟動服務
        print(f"{Colors.HEADER}[系統] 啟動服務...{Colors.ENDC}")
        
        backend_process = run_backend(python_path)
        frontend_process = run_frontend()
        
        print(f"{Colors.GREEN}[系統] 所有服務已啟動！{Colors.ENDC}")
        print(f"{Colors.GREEN}[系統] 前端地址: http://localhost:3000{Colors.ENDC}")
        print(f"{Colors.GREEN}[系統] API 地址: http://localhost:8000{Colors.ENDC}")
        print(f"{Colors.GREEN}[系統] API 文檔: http://localhost:8000/docs{Colors.ENDC}")
        print(f"{Colors.YELLOW}[系統] 按 Ctrl+C 停止服務{Colors.ENDC}")
        
        # 等待進程結束
        while True:
            time.sleep(1)
            if backend_process.poll() is not None:
                print(f"{Colors.RED}[後端] API 服務器已停止{Colors.ENDC}")
                break
            if frontend_process.poll() is not None:
                print(f"{Colors.RED}[前端] Next.js 應用已停止{Colors.ENDC}")
                break
                
    except KeyboardInterrupt:
        print(f"\n{Colors.YELLOW}[系統] 收到中斷信號{Colors.ENDC}")
    except Exception as e:
        print(f"{Colors.RED}[錯誤] 啟動失敗: {e}{Colors.ENDC}")
    finally:
        cleanup(None, None)
